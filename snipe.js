...
const config = {
  ...
  jupiter: {
    slippageBps: parseInt(process.env.SLIPPAGE_BPS) || 100, // 1% default
    priorityFee: parseInt(process.env.PRIORITY_FEE) || 10000, // microlamports
  },
  alerts: {
    checkInterval: parseInt(process.env.ALERT_INTERVAL) || 30000, // 30 seconds
  }
};
...
class BotState {
  constructor() {
    ...
    this.priceAlerts = new Map(); // alertId -> AlertData
    this.tokenCache = new Map(); // mint -> TokenInfo
    this.alertCounter = 0;
  }
...
🤖 *Welcome to Grokini Trading Bot v3.0*

Your ultimate Solana trading companion!
...
bot.on('callback_query', async (query) => {
  ...
  switch (data) {
    case 'main_menu':
      await bot.editMessageText('🤖 *Grokini Trading Bot*\n\nSelect an option:', {
        chat_id: chatId,
